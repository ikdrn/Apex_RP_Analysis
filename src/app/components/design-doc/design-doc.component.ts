...
本システムは、ぺこんぽプレイヤーの RP を定期的に自動取得・蓄積し、日々・時間帯ごとの RP 変化をグラフと表で振り返れるようにすることを目的として開発された。
「今どのくらいか」だけでなく「この 1 週間でどう変化したか」「ピーク帯の成績は？」を即座に把握できる環境を提供する。`
        },
        {
          label: '業務要件',
          content: `① RP 自動取得：毎日 21:00〜翌 03:00 の間、15 分ごとに外部 API（Apex Legends Status API）からぺこんぽプレイヤーの RP を取得し Supabase に保存する。
...
FR-04 データテーブル表示：取得レコード一覧（連番 id、rp、created_at）を日付ソート（昇順/降順）で表示する。
...
・取得対象プレイヤーはぺこんぽのみ（マルチプレイヤー対応は対象外）`
        },
        {
          label: '業務フロー',
          content: `【As-Is（現状）】
...
Job-01: RP 取得バッチ
  ...
  処理: Apex Status API からぺこんぽの RP を取得し player_rp に INSERT
...
ST-A-04: Data Table タブでデータ一覧が表示され、日付ソートが動作する
...
ST-D-03: レートリミット超過時（61 req/min）に 429 が返ること`
        ...
        {
          label: 'テストデータ',
          content: `本番データを使用（実際のぺこんぽの RP データ）
...
【ライブラリバージョン】
  ...
  toKeyValueRows(content: string): Array<{ key: string; value: string }> {
    const lines = content
      .split('\n')
      .map((line) => line.trim())
