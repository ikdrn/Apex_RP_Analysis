...
export class DesignDocComponent {
  selectedSectionId: DesignDocSectionId = 'requirements';
...
FR-04 データテーブル表示：取得レコード（id, rp, created_at）を一覧表示する。
FR-05 CSV ダウンロード：表示中データを id,rp,created_at の CSV ファイルとして出力する。
FR-06 最新データ更新：ボタン操作で API を再取得する。
FR-07 設計書閲覧：本設計書を Web 画面内タブで参照できる。
FR-08 エラー表示：データ取得失敗時に日本語エラーメッセージを表示する。`
        },
        {
          label: '非機能要件',
          content: `【性能】
...
FN-04 データテーブル表示（ページングなし、全件表示）
FN-05 CSV ダウンロード
FN-06 最新データ更新ボタン
FN-07 設計書タブ表示
FN-08 エラー表示（API 失敗時）
FN-09 RP 定期取得バッチ（pg_cron / 15 分間隔 / 21:00〜03:00 JST）
FN-10 古レコード削除バッチ（pg_cron / 30 日超過分を自動削除）`
        },
        {
          label: '画面設計（遷移図・レイアウト）',
          content: `【画面遷移図】
アプリ起動
  └→ メイン画面（初期タブ: Analysis）
       ├─ [Analysis タブ] RP グラフ表示
       ├─ [Data Table タブ] RP データ一覧
       └─ [設計書 タブ] 設計書閲覧

タブ間は同一画面内で遷移（SPA、ルーティングなし）

【画面レイアウト】
Header: タイトル「Apex RP Analysis」＋「設計書を見る」リンク
Period Selector: 7日 / 30日 ボタン ＋ 更新ボタン ＋ CSV ボタン
Stats Cards: 最新RP / 最高RP / 最低RP / 変化量 / 平均RP / 1日あたり変化（6カード）
Main Tab Area: Analysis / Data Table / 設計書 タブパネル
...
URL: /api/get-rp
クエリパラメータ: days=7|30
レスポンス型: RpRecord[] = { id: string, rp: number, created_at: string }[]
...
注意: 削除前にレコード件数を確認する場合は手動で COUNT クエリを実行`
        },
        {
          label: 'オンライン処理設計',
...
エラー発生時: this.error に日本語メッセージをセット
  ...
  → detail = err.error.error または err.message
...
プロパティ:
  activeTab: 'analysis' | 'table' | 'design'  現在のアクティブタブ
  loading: boolean                               初回ロード中フラグ
  refreshing: boolean                            再取得中フラグ
  error: string                                  エラーメッセージ
  records: RpRecord[]                            取得 RP レコード配列
  selectedRange: 7 | 30                          選択中の表示期間
  lineChartData: ChartConfiguration<'line'>['data']  グラフデータ
  lineChartOptions: ChartConfiguration<'line'>['options']  グラフオプション

算出プロパティ（getter）:
  latestRp: number | null    最終レコードの RP
  maxRp: number | null       最大 RP
  minRp: number | null       最小 RP
  rpChange: number | null    最初〜最後の差分
  avgRp: number | null       平均 RP
  rpPerDay: number | null    1 日あたり変化量

メソッド:
  ngOnInit()           初期化（loadRecords 呼び出し）
  loadRecords(isRefresh?: boolean)  API 呼び出し・グラフ更新
  onRangeChange(days)  期間変更
  refresh()            再取得
  downloadCsv()        CSV 出力
  showDesignDoc()      設計書タブへ遷移`
        },
        {
          label: 'インターフェース設計（詳細・シーケンス図）',
          content: `【シーケンス図: RP データ取得フロー】
Browser       Angular       Vercel API    Supabase
   │             │               │             │
   │─ 画面表示 →│               │             │
   │             │─ GET /api/get-rp?days=30 →│ │
   │             │               │─ GET /rest/v1/player_rp?... →│
   │             │               │             │← JSON 配列 ─────│
   │             │← JSON 配列 ──│             │
   │← グラフ描画│               │             │
...
タイムアウト設定:
  ...
  fetch（API→Supabase）: Node.js デフォルト（接続待機無制限、個人用途のため許容）
...
【インデックス定義】
  ...
  種別: B-Tree
  列: created_at DESC
  用途: 期間フィルタクエリの高速化（created_at >= 'since'）

【ビュー・ストアドプロシージャ・トリガー】
  なし（シンプル構成）

【データベース制約】
  NOT NULL: id, rp, created_at
  PK: id (UUID)

【データ容量見積もり】
  1レコードのサイズ: 約 60〜80 bytes（UUID 16B + INT 4B + TIMESTAMPTZ 8B + オーバーヘッド）
  30日分最大件数: 約 4,350 件（15分×145回×30日）
...
UT-C-02: selectedSection — 存在しない ID → undefined`
  ...
  { id: 'aaa-111', rp: 1500, created_at: '2026-03-01T12:00:00+09:00' }
  { id: 'bbb-222', rp: 1600, created_at: '2026-03-02T12:00:00+09:00' }
  { id: 'ccc-333', rp: 1400, created_at: '2026-03-03T12:00:00+09:00' }
...
IT-A-04: Supabase に存在しない日付範囲を指定 → 空配列が返り「データなし」表示
...
IT-B-05: Analysis → Data Table → 設計書 タブ切替が正常に動作する
...
・IT-C: エラー時に UI 上にエラーメッセージが表示されること（コンソールエラーのみでは不可）
...
ST-A-04: Data Table タブでデータ一覧が表示される
ST-A-05: CSVダウンロードでファイルが取得でき、データが正しい
ST-A-06: 設計書タブで全 7 セクションが閲覧できる
...
ST-D-01: SUPABASE_SERVICE_ROLE_KEY がクライアントの JS ファイル・レスポンスに露出していないこと
ST-D-02: 不正な days パラメータ（/api/get-rp?days=DROP TABLE）が 30 日にフォールバックされ安全に動作すること
...
② 各機能テスト（ST-A-01〜06）を順に実行し、スクリーンショットで記録する
...
ブランチ戦略:
  main（master）: 本番リリース済みコード
  claude/{feature-name}-{id}: 機能追加・修正ブランチ（Claude Code 作業用）
  PR: GitHub Pull Request でレビュー後 master にマージ
...
③ master ブランチへ Squash Merge または Merge Commit でマージする
④ Vercel が master ブランチの push を検知して自動ビルド・デプロイを実行する
⑤ Vercel Dashboard でデプロイが成功したことを確認する
⑥ 本番 URL で総合テスト（ST-A-01〜06）を実施する
...
③ master ブランチへの push で自動デプロイが有効になることを確認する
...
【ライブラリバージョン】
  ...
  selectSection(sectionId: DesignDocSectionId): void {
